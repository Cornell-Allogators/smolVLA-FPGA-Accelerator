...
As a result, we approximate the GELU (Gaussian Error Linear Unit) using a hyperbolic tangent ($\tanh$) formulation:$ "GELU"(x) approx frac(1, 2) dot x dot (1 + tanh(sqrt(2/pi) * (x + 0.044715 dot x^3))) $. This formulation is itself an approximation. We express the $\tanh$ function using a polynomial approximation, often based on Cody and Waite's rational form. This particular approximation for $\tanh$ requires, on-chip, 4 floating-point multiplications ($\text{fmul}$), 3 additions ($\text{fadd}$), and 1 division ($\text{fdiv}$) in single precision. Combined with the non-$\tanh$ operations (2 $\text{fadd}$, 6 $\text{fmul}$, 1 $\text{fdiv}$), the entire GELU calculation requires 16 operations.
